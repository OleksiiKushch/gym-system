...
class TrainingUnitTest {
    ...
    void shouldInitByCsvRecord() {
        ...
        when(csvRecord.get(3)).thenReturn(TrainingTypeEnum.CARDIO.name());
        ...
        assertEquals(TEST_TRAINEE_ID, testInstance.getTrainee().getUserId().toString());
        assertEquals(TEST_TRAINER_ID, testInstance.getTrainer().getUserId().toString());
        assertEquals(TEST_TRAINING_NAME, testInstance.getTrainingName());
        assertEquals(TrainingTypeEnum.CARDIO, testInstance.getTrainingType().getName());
